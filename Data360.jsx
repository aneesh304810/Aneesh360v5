    ...
    <div>
      ...
      {(m.column_lineage || []).length === 0 ? (
        <div style={{ fontSize: 12, color: t.muted, padding: "10px 0" }}>
          No column-level lineage captured for this model. (Parsed by sqlglot from the model SQL —
          if empty, the model's SQL may not have been analyzed yet.)</div>
      ) : (() => {
        // classify each edge by transform type for the colored pill
        ...
        const typeOf = (expr, from) => {
          ...
          if (/CURRENT_DATE|SYSDATE|'[^']*'/.test(e) && !/\(/.test(e)) return ["literal", "#5a6472"];
        ...
        return (
          <div style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 8,
            padding: 16, marginBottom: 8, display: "flex", flexDirection: "column", gap: 8 }}>
            {rows.map((c, i) => {
              ...
              return (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 10 }}>
                  <div style={{ fontFamily: "monospace", fontSize: 12, textAlign: "right",
                    background: t.bgsoft || "#eef3f3", padding: "5px 10px", borderRadius: 4 }}>{c.from_column || "—"}</div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: tcol,
                    padding: "4px 12px", borderRadius: 14, whiteSpace: "nowrap", textAlign: "center" }}>
                    {c.transform_expr && c.transform_expr !== "1:1"
                      ? (c.transform_expr.length > 26 ? tlabel : c.transform_expr) : "1:1 passthrough"}</div>
                  <div style={{ fontFamily: "monospace", fontSize: 12,
                    border: `1px solid ${t.border}`, padding: "5px 10px", borderRadius: 4, color: t.navy }}>{c.to_column}</div>
                </div>);
            })}
            <div style={{ display: "flex", gap: 14, marginTop: 6, fontSize: 10, color: t.sub }}>
              {[["passthrough", "#5a7a8a"], ["function", "#d17a2a"], ["derived (multi-source)", "#0f4775"], ["literal", "#5a6472"]].map(([l, c]) => (
