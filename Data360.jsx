  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 2, borderBottom: `1px solid ${t.border || t.disabled}`,
        margin: "18px 0 20px" }}>
        {["Pipelines", "Inbound Feeds", "Loaders", "Compression", "Lineage Graph"].map((tb) => (
...
function ModelDetail({ t, m, LAYER, onBack, embedded }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: t.muted || t.textMuted, margin: "16px 0 8px" }}>
        Column-level transformation — graph view</div>
      {(m.column_lineage || []).length === 0 ? (
        <div style={{ fontSize: 12, color: t.muted, padding: "10px 0" }}>
          No column-level lineage captured for this model.</div>
      ) : (() => {
        const rows = m.column_lineage || [];
        const shortCol = (s) => {
          if (!s) return "—";
          const parts = String(s).split(".");
          return parts[parts.length - 1];   // strip dbt.imdw.model. prefix
        };
        const typeOf = (expr, from) => {
          const e = (expr || "").toUpperCase().trim();
          if (!e || e === "1:1" || e === (from || "").toUpperCase()) return ["passthrough", "#5a7a8a"];
          if (/\bCASE\b|\bWHEN\b/.test(e)) return ["derived", "#0f4775"];
          if (/\w+\s+[*+/-]\s+\w+/.test(e) || e.includes(" * ") || e.includes(" + ")) return ["derived", "#0f4775"];
          if (/CURRENT_DATE|SYSDATE/.test(e) || (/^'.*'$/.test(e) && !e.includes("("))) return ["literal", "#5a6472"];
          if (/\w+\s*\(/.test(e)) return ["function", "#d17a2a"];
          return ["passthrough", "#5a7a8a"];
        };
        const ROW_H = 46, PAD = 14, W = 860;
        const xSrc = 150, xPill = W / 2, xTgt = W - 150;
        const H = rows.length * ROW_H + PAD * 2;
        return (
          <div style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 8,
            padding: 12, marginBottom: 8, overflowX: "auto" }}>
            <svg width={W} height={H} style={{ minWidth: W }}>
              {/* header labels */}
              <text x={xSrc} y={12} fontSize={10} fontWeight={700} fill={t.sub} textAnchor="middle">SOURCE</text>
              <text x={xTgt} y={12} fontSize={10} fontWeight={700} fill={t.sub} textAnchor="middle">TARGET</text>
              {rows.map((c, i) => {
                const y = PAD + i * ROW_H + ROW_H / 2 + 8;
                const [tlabel, tcol] = typeOf(c.transform_expr, c.from_column);
                const pillText = !c.transform_expr || c.transform_expr === "1:1"
                  ? "1:1" : (c.transform_expr.length > 22 ? tlabel : c.transform_expr);
                const pillW = Math.max(50, pillText.length * 7 + 18);
                return (
                  <g key={i}>
                    {/* edge: source -> pill -> target */}
                    <path d={`M${xSrc + 60},${y} C${(xSrc + xPill) / 2},${y} ${(xSrc + xPill) / 2},${y} ${xPill - pillW / 2},${y}`}
                      stroke={tcol} strokeWidth={1.5} fill="none" />
                    <path d={`M${xPill + pillW / 2},${y} C${(xPill + xTgt) / 2},${y} ${(xPill + xTgt) / 2},${y} ${xTgt - 70},${y}`}
                      stroke={tcol} strokeWidth={1.5} fill="none"
                      strokeDasharray={tlabel === "literal" ? "4,3" : "none"} />
                    {/* source column box */}
                    <rect x={xSrc - 62} y={y - 13} width={124} height={26} rx={4}
                      fill={t.bgsoft || "#eef3f3"} stroke={t.border} />
                    <text x={xSrc} y={y + 4} fontSize={11} fontFamily="monospace" fill={t.navy} textAnchor="middle">
                      {shortCol(c.from_column).slice(0, 16)}</text>
                    {/* transform pill */}
                    <rect x={xPill - pillW / 2} y={y - 11} width={pillW} height={22} rx={11} fill={tcol} />
                    <text x={xPill} y={y + 4} fontSize={10} fontWeight={700} fill="#fff" textAnchor="middle">{pillText}</text>
                    {/* target column box */}
                    <rect x={xTgt - 68} y={y - 13} width={136} height={26} rx={4}
                      fill={t.panel} stroke={t.border} />
                    <text x={xTgt} y={y + 4} fontSize={11} fontFamily="monospace" fill={t.navy} textAnchor="middle">
                      {shortCol(c.to_column).slice(0, 18)}</text>
                  </g>);
              })}
            </svg>
            <div style={{ display: "flex", gap: 14, marginTop: 4, fontSize: 10, color: t.sub, paddingLeft: 12 }}>
              {[["passthrough", "#5a7a8a"], ["function", "#d17a2a"], ["derived", "#0f4775"], ["literal", "#5a6472"]].map(([l, c]) => (
                <span key={l} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                  <span style={{ width: 14, height: 3, background: c, borderRadius: 2 }} />{l}</span>))}
            </div>
          </div>);
      })()}

      <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: t.muted || t.textMuted, margin: "16px 0 8px" }}>
        Column-level transformation rules</div>
      <table style={{ width: "100%", borderCollapse: "collapse", background: t.panel, border: `1px solid ${t.border}`, borderRadius: 4, overflow: "hidden" }}>
        <thead><tr>{["Source column(s)", "→", "Target column", "Transform rule"].map((h) => (
        ...
        <tbody>{(m.column_lineage || []).map((c, i) => (
          <tr key={i}>
            ...
            <td style={{ padding: "8px 10px", fontFamily: "monospace", fontSize: 12, color: t.navy, borderBottom: `1px solid ${t.bg}` }} title={c.to_column}>{String(c.to_column || "").split(".").pop()}</td>
          ...
          {(m.column_lineage || []).length === 0 && <tr><td colSpan={4} style={{ padding: "10px", fontSize: 12, color: t.muted }}>No column rules.</td></tr>}</tbody>
      </table>
        ...
        <div style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 8, overflow: "hidden", maxHeight: 540, overflowY: "auto" }}>
          {(() => {
            ...
            return domains.map((d) => (
              <div key={d}>
                <div style={{ padding: "7px 14px", background: t.bgsoft || "#eef3f3",
                  fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".5px",
                  color: t.accent, position: "sticky", top: 0 }}>
        ...
        {loaders.map((l) => (
          <button key={l.loader_id} onClick={() => pick(l.loader_id)}
            ...
            <div style={{ fontSize: 10.5, color: t.sub || t.textMuted, marginTop: 2 }}>
              {[l.group_name, l.business_domain, l.version && `v${l.version}`].filter(Boolean).join(" \u00b7 ")}</div>
          </button>))}
      </div>
