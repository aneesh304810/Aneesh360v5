          ...
          {flowMode === "Runbooks" && (
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              ...
              <div style={{ width: 280, flexShrink: 0 }}>
                ...
                {bizFlows.filter((f) => !flowSearch.trim() ||
                  (f.display_name || "").toLowerCase().includes(flowSearch.toLowerCase()) ||
                  (f.domain || "").toLowerCase().includes(flowSearch.toLowerCase())).map((f) => (
                  <button key={f.flow_id}
                    onClick={() => { setBuilding(false);
                      f._src === "bf"
                        ? api.bfApiFlow(f.flow_id).then((d) => setActiveBiz(_bfToBiz(d)))
