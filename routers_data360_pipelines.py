...
def model_detail(pipeline_id: str, model_name: str):
    ...
    cols = _safe("""SELECT from_column, to_column, transform_expr, source, model_key
        FROM column_lineage
        WHERE LOWER(model_key) LIKE :m OR LOWER(model_key) = :exact
        ORDER BY to_column""",
        {"m": f"%{model_name.lower()}%", "exact": model_name.lower()})
